...
const { handleTextCommand, handlePendingUserCostSetup, handlePendingWorklogSetup } = require('./slackCommands');
...
app.event('message', async ({ event, client }) => {
  try {
    ...
    if (isDmChannel) {
      ...
      const handledWorklogSetup = await handlePendingWorklogSetup(event, client);
